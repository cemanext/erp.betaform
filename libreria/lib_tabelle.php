                    ...
                    switch (strtolower($nome_colonna)) {
                        ...
                        case "id_prodotto":
                            if($tabella == "lista_preventivi_dettaglio" OR $tabella == "lista_fatture_dettaglio")
                                $rowTable .=  '<td style="text-align:center; vertical-align:middle;">' . print_select2_group("SELECT id as valore, nome, gruppo, prezzo_pubblico AS var_1, iva AS var_2, if(quantita>0,quantita,1) AS var_3, $record AS var_4 FROM lista_prodotti WHERE stato='Attivo' ORDER BY gruppo ASC, nome ASC", "txt_" . $record . "_" . $nome_colonna, $column, "scriviDentroListaPreventiviDettaglioTXT", false, 'tooltips select_prodotto', 'data-container="body" data-placement="top" data-original-title="PRODOTTO"') . '</td>';
                            else if($tabella == "lista_prodotti_dettaglio")
                                $rowTable .=  '<td style="text-align:center; vertical-align:middle;">' . print_select2_group("SELECT id as valore, nome, gruppo, prezzo_pubblico AS var_1, iva AS var_2, if(quantita>0,quantita,1) AS var_3, $record AS var_4 FROM lista_prodotti WHERE stato='Attivo' AND gruppo='CORSO' ORDER BY gruppo ASC, nome ASC", "txt_" . $record . "_" . $nome_colonna, $column, "scriviDentroListaPreventiviDettaglioTXT", false, 'tooltips select_prodotto', 'data-container="body" data-placement="top" data-original-title="PRODOTTO"') . '</td>';
                            else
