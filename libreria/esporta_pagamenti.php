...
$rsDatiPagamento = $dblink->get_results("SELECT * FROM lista_costi WHERE id_fattura > 0 AND YEAR(data_creazione) = '$anno' AND MONTH(data_creazione) = '$mese'");

$righe = "";

foreach ($rsDatiPagamento  as $rowDatiPagamento) {
        
    if(!empty($rowDatiPagamento)){
        
        $rowFattura = $dblink->get_row("SELECT * FROM lista_fatture WHERE id = '".$rowDatiPagamento['id_fattura']."'", true);
        ...
        if(!empty($rowFattura)){
            ...
            if(strtolower($rowFattura['tipo'])=="fattura"){
                $riga .= aggiungiSpaziAllaStringa("048", 3, true, "0");  //TRF-CAUSALE
                $riga .= aggiungiSpaziAllaStringa("INCASSO FATTURA", 15, true);  //TRF-CAU-DES
            }else{
                $riga .= aggiungiSpaziAllaStringa("048", 3, true, "0");  //TRF-CAUSALE
                $riga .= aggiungiSpaziAllaStringa("INCASSO FATTURA", 15, true);  //TRF-CAU-DES
            }
            ...
            $riga .= aggiungiSpaziAllaStringa(str_replace("-", "", GiraDataOra($rowDatiPagamento['data_creazione'])), 8, true, "0");  //TRF-DATA-REGISTRAZIONE
            $riga .= aggiungiSpaziAllaStringa("", 8, false, "0");  //TRF-DATA-DOC
            $riga .= aggiungiSpaziAllaStringa("", 8, false, "0");  //TRF-NUM-DOC-FOR
            $riga .= aggiungiSpaziAllaStringa("", 5, false, "0", false);  //TRF-NDOC
            $riga .= aggiungiSpaziAllaStringa("", 2, false, "0", false);  //TRF-SERIE
            ...
            //Dati Iva
            $riga .= aggiungiSpaziAllaStringa("", 12, true, "0", false);  //TRF-IMPONIB
            $riga .= aggiungiSpaziAllaStringa("", 3, true, "0", false);  //TRF-ALIQ
            $riga .= aggiungiSpaziAllaStringa("", 3, false, "0");  //TRF-ALIQ-AGRICOLA
            $riga .= aggiungiSpaziAllaStringa("", 2, false, "0");  //TRF-IVA11
            $riga .= aggiungiSpaziAllaStringa("", 11, true, "0", false);  //TRF-IMPOSTA

            //SPAZIO VUOTO CON 0
            $riga .= aggiungiSpaziAllaStringa("", 217, false, "0"); //SPAZIO VUOTO ???

            //Totale fattura
            $riga .= aggiungiSpaziAllaStringa("", 12, true, "0", false);  //TRF-TOT-FATT

            //Conti di ricavo/costo
            $riga .= aggiungiSpaziAllaStringa("", 7, false, "0");  //TRF-CONTO-RIC
            $riga .= aggiungiSpaziAllaStringa("", 12, true, "0", false);  //TRF-IMP-RIC

            //SPAZIO VUOTO CON 0
            $riga .= aggiungiSpaziAllaStringa("", 133, false, "0"); //SPAZIO VUOTO ???
            

            //$riga .= aggiungiSpaziAllaStringa("", 601, false); //SPAZIO VUOTO ???

            //Dati eventuale pagamento fattura o movimenti diversi
            //if(!empty($rsDatiPagamento)){
            $riga .= aggiungiSpaziAllaStringa("48", 3, true, "0", false);  //TRF-CAU-PAGAM
            if(strtolower($rowFattura['tipo'])=="fattura"){
                $riga .= aggiungiSpaziAllaStringa("INCASSO FATTURA", 15, true);  //TRF-CAU-DES-PAGAM
            }else{
                $riga .= aggiungiSpaziAllaStringa("INCASSO FATTURA", 15, true);  //TRF-CAU-DES-PAGAM
            }
            ...
            $riga .= aggiungiSpaziAllaStringa("9999999", 7, true, "0");  //TRF-CONTO
            if(strtolower($rowFattura['tipo'])=="fattura"){
                $riga .= aggiungiSpaziAllaStringa("A", 1, true);  //TRF-DA
            }else{
                $riga .= aggiungiSpaziAllaStringa("D", 1, true);  //TRF-DA    
            }
            $riga .= aggiungiSpaziAllaStringa(str_replace(".","", sprintf('%0.2f', abs($rowDatiPagamento['entrate']))), 12, true, "0", false);  //TRF-IMPORTO
            $riga .= aggiungiSpaziAllaStringa($rowFattura['codice'].SEPARATORE_FATTURA.$rowFattura['sezionale']." ".GiraDataOra($rowFattura['data_creazione']), 18, true);  //TRF-CAU-AGGIUNT
            ...
            $numGiriPag = 0;
                ...
                //foreach ($rsDatiPagamento as $rowDatiPagamento) {
                    $riga .= aggiungiSpaziAllaStringa("2405501", 7, true, "0");  //TRF-CONTO
                    if(strtolower($rowFattura['tipo'])=="fattura"){
                        $riga .= aggiungiSpaziAllaStringa("D", 1, true);  //TRF-DA
                    }else{
                        $riga .= aggiungiSpaziAllaStringa("A", 1, true);  //TRF-DA    
            ...
            $riga .= aggiungiSpaziAllaStringa("", (5056-$righeFattePag), false); //SPAZIO VUOTO ???
